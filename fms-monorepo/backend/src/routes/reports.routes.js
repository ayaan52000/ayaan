...
router.get("/expenses.csv", async (req, res, next) => { try { const parsed = rangeSchema.parse(req.query); const rows = await prisma.expense.findMany({ where: filters(req, parsed), include: { branch: true, creator: true, category: true }, orderBy: { createdAt: "desc" } }); sendCsv(res, "expenses.csv", ["ID", "Date", "Branch", "Creator", "Category", "Description", "Amount", "Status", "Receipt"], rows.map((item) => [item.id, item.createdAt.toISOString(), item.branch.name, item.creator.name, item.category.name, item.description, item.amount, item.status, item.receiptUrl])); } catch (error) { next(error); } });
