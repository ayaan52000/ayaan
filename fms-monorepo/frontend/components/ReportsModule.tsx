...
export default function ReportsModule({ role }: { role: Role }) {
  ...
  async function download(path: string) {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/reports/${path}?${params}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
